document.addEventListener('DOMContentLoaded', function () {
  ...
  const filename = fileInput.files[0]?.name || 'Unnamed File';
  const scanDate = new Date().toLocaleString();
  const complexity = complexityBox.textContent || 'Estimated Time Complexity: N/A';

  const reportHeader = `
    <div style="margin-bottom: 20px;">
      <h2>Algorithm Scanner Report</h2>
  ...
  const reportFooter = `
    <div style="margin-top: 30px; font-size: 0.8em; color: #666;">
      <p>Generated by Algorithm Scanner Pro</p>
    </div>
  `;

  const element = document.createElement('div');
  element.innerHTML = reportHeader + document.querySelector('.code-wrapper').outerHTML + reportFooter;

  const opt = {
    margin: 0.5,
    filename: 'algorithm_report.pdf',
